...
Here I solve equation~\ref{rdashdash} numerically, using R programming
language~\cite{rcore2019} and the \verb+deSolve+ package for ordinary
differential equations~\cite{soetart2010}.  Non-radial solutions fall
into two distinct classes: those with $r>2M$ at all points, and those
which attain $r=2M$, in which case the string is tangential to the
event horizon.
...
\centering
\includegraphics[width=\linewidth]{one_free_end_fixed_EH_intersection.pdf}
...
\centering
\includegraphics[width=\linewidth]{one_free_end_r_equals_2.pdf}
