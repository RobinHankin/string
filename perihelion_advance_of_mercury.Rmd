...
expensive, but we can plot the simulated trajectory close to aphelion
as for the Newtonian case:
...
shown as a solid circle, subseqent points shown with open circles.
...
orbits, and the time evolution is not shown.  To proceed we need to
find the aphelion (or an approximation to it) somehow, and show that
the aphelion systematically increases with time.  Given the numerics,
we find points on the simulated trajectory which are a local maximum
distance from the sun.  This is straightfoward numerically, we
identify iterations $i$ (which have radius `r[i]`) satisfying both
`r[i-1]<r[i]` and `r[i]>r[i+1]`:
