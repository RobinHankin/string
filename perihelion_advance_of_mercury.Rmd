...
years.  According to Einstien and others, the general theory of
...
visible deviation from an exactly closed orbital path.  To calculate
the precession we need to simulate the motion for a longer time:
...
We need to isolate the change points (actually calculating aphelion
rather than perihelion for convenience):
...
To convert this figure which is given in radians per orbit) to
arcseconds per century, we need to multiply by a sequence of factors:
