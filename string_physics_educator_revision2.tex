...
Schwarzschild $r$ between string and null geodesic, we see that
$d''=M$ and with appropriate boundary conditions
$d=\frac{Mx^2}{2r_0^2}$, where $x=r_0\theta$ is an appropriate
circumferential coordinate.  However, to a static local observer tied
to the string would
$d_\mathrm{local}=Mx^2/\left(2r_0^2\sqrt{1-2M/r_0}\right)$,

$d_\mathrm{local}=d/\sqrt{1-2M/r_0}$,


Thus a taut string would {\em look} curved to a static
observer in Schwarzschild coordinates, having a (visual) radius of
curvature of
