...
approach is less than about $1.0761\cdot 2GM$, the
string self-intersects.  If the string attains $r=2GM$, then it
...
ds^2= -dt^2\left(1-2GM/r\right) +\frac{dr^2}{1-2GM/r} + r^2\left(d\theta^2 + \sin^2\theta d\phi^2\right),
\end{equation}

\noindent where units in which~$c=1$ are used, has been known for
over a century~\cite{schwarzschild1916}.  Here I consider the spatial
component~$ds^2= dr^2/\left(1-2GM/r\right) + r^2d\phi^2$, confined to
equatorial points~($\theta=\pi/2$); we understand~$2GM\leq r<\infty$
...
from~$r=r_1\geq 2GM$ to~$r_2\geq r_1$ at constant $\theta,\phi$:
...
\begin{equation}\label{radial_string_length}
  \int_{r=r_1}^{r_2}\frac{dr}{\sqrt{1-2GM/r}}=
  \left.
  \sqrt{r(r-2GM)} +\frac{1}{2}\log\left(
  \frac{\sqrt{r}+\sqrt{r-2GM}}{\sqrt{r}-\sqrt{r-2GM}}\right)
  \right|_{r_1}^{r_2}
  \end{equation}

Observe that integration may be performed with a lower limit
of~$r_1=2GM$ with no difficulty, even though the radial component of the
...
$r=2GM$ to $r=2GM(1+w)$ where $w>0, \left|w\right|\ll 1$.  Then
 ...
 \begin{equation}\label{ell}
   \ell =
   \int_{r=2GM}^{r=2GM(1+w)}ds
   =2GM\left(2\sqrt{w} + \frac{1}{3}w^{3/2} + O(w^{5/2})\right).
 \end{equation}

This would indicate that, close to the Schwarzschild radius $r=2GM$,
...
\begin{equation}\label{asymptotic_ell}
  \ell = 2GM\left(w +  \frac{1}{2}\log w + \frac{1}{2} + \log(2)  +  \frac{1}{8}w^{-1} + O(w^{-2})\right),
\end{equation}

\noindent suggesting that there is ``extra radial space'', of length
over $1.19\cdot 2GM$, sensible at large distances from the event
...
\begin{equation}
%  \int_{p_1}^{p_2}\sqrt{\frac{\left(r'\right)^2}{1-2M/r} + r^2}d\phi=
  \int_{p_1}^{p_2}\left(\frac{\left(r'\right)^2}{1-2GM/r} + r^2\right)^\frac{1}{2}d\phi=
  \int_{p_1}^{p_2}F\left(r,r'\right)d\phi
\end{equation}
...
\begin{equation}\label{rdashdash}
  r''\left(\phi\right) =
  (r-2GM) + \frac{(4r-6GM)\left(r'\right)^2}{2r\left(r-2GM\right)},\qquad r>2GM.
\end{equation}
...
geodesics, which is usually given~\cite{wald} in terms of $u=2GM/r$ as
$u''=3u^2/2-u$, but is equivalent
to~$r''\left(\phi\right)=r-3GM+4GMr'/r$.
...
solutions: those with a tangential point ($r'=0,r>2GM$), and those
without.  Consider first solutions with such a point: there exists a
pair~$\left(r,\phi\right), r>2GM$ at which~$r'(\phi)=0$.  This point
...
%    \right|
  ...
  \frac{
    ...
    r-\frac{\left(r'\right)^2}{2\left(r-2GM\right)}
  }\qquad\mbox{along a taut string}
\end{equation}

\noindent which would give~$R$ as a function of~$\left(r,r'\right)$.
Considering~$r'(\phi)=0$ shows that if a string is tangential at a
point with~$r_0>2GM$, then it has a radius of curvature of~$r_0^2/2GM$
at that point.  Any circular path, including a circular orbit
at~$r=r_0$, has a radius of curvature of $r_0<r_0^2/2GM$, showing that
circular orbits cannot follow a taut string at closest approach.  Also
observe that null geodesics have a radius of curvature of $r_0^2/3GM <
r_0^2/2GM$, showing that light cannot follow a taut string at closest
approach either.  The string has a point of inflection
if~$\left(r'\right)^2=2r(r-2GM)$.
...
If, conversely, $r'(\phi)\neq 0$ whenever~$r>2GM$, then the string has
...
solutions fall into two distinct classes: those with $r>2GM$ at all
points, and those which attain $r=2GM$, in which case the string is
tangential to the event horizon.

We first consider strings for which $r>2GM$ everywhere (that is,
...
2GM$, the string does not self-intersect; if the closest approach is
less than this value, then the string crosses itself at~$\phi=\pi/2$.
As discussed above, if $\left(r'\right)^2=2r(r-2GM)$, the string has a
...
through~$(4GM,0)$.  Figure~\ref{light_r_equals_2} shows light paths for
comparison.

The cases where the string attains $r=2GM$ are tangent to the event
...
shows tangential strings passing through $(4GM,0)$.
...
\caption{Self-intersecting strings shown for $\phi\geq 0$.  Closest
  ...
  $r/2GM\in\left(1,1.0761\right)$; strings are symmetrical about $\phi=0$
...
\caption{Strings passing through $(4GM,0)$}
...
\caption{Null geodesics passing through $(4GM,0)$, shown for $0\leq\phi\leq 2\pi$}
...
\caption{Strings tangential to the event horizon passing through $(4GM,0)$}
